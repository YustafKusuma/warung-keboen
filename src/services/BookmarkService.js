...
const addBookmark = async ({categoryId}) => {
  console.log(`BookmarkService | addBookmark`);
  try {
    let response = await axios.post(
      `${ApiContants.BACKEND_API.BASE_API_URL}${ApiContants.BACKEND_API.BOOKMARK}/${categoryId}`,
      {},
      {
        headers: authHeader(getToken()),
      },
...
const removeBookmark = async ({categoryId}) => {
