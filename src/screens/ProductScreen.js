...
import {ProductService, StaticImageService} from '../services';
...
const ProductScreen = ({
  ...
  const itemCount = useSelector(
    state =>
      state?.cartState?.cart?.cartItems?.find(item => item?.productId === productId)
        ?.count,
  );

  useEffect(() => {
    ProductService.getOneProductById(productId).then(response => {
          ...
          <View style={styles.detailsContainer}>
            ...
            {product?.other ? (
              <>
                ...
                <Text style={styles.detailContent}>{product?.other}</Text>
              </>
            ) : null}
          </View>
        </View>
